...
def export_to_excel(invoice_data: dict, output_path: str) -> None:
    """
    ...
    kopf_data = {
        "Feld": [
            ...
            "Lieferant",
            "Währung",
            f"Betrag netto ({currency})",
            f"MwSt. ({currency})",
            f"Betrag brutto ({currency})",
            "IBAN",
        ],
        "Wert": [
            invoice_data.get("rechnungsnummer"),
            invoice_data.get("datum"),
            invoice_data.get("lieferant"),
            currency,
            invoice_data.get("betrag_netto"),
            invoice_data.get("mwst"),
            invoice_data.get("betrag_brutto"),
    ...
    if positionen:
        ...
        df_positionen = pd.DataFrame(
            columns=[
                "Artikel",
                "Menge",
                f"Einzelpreis ({currency})",
                f"Gesamtpreis ({currency})",
            ]
        )
...
def export_multiple_to_excel(invoices: list[dict], output_path: str) -> None:
    """
    Export multiple invoices into a single Excel workbook.

    Sheet 1: Rechnungen (one row per invoice)
    Sheet 2: Positionen (all line items across invoices)

    Each row on both sheets includes a "source_file" column so individual
    invoices/items can be traced back to the PDF they came from. Amount
    columns intentionally do not include the currency in their name (since
    invoices may use different currencies); a "currency" column is included
    instead.

    Args:
        invoices: List of parsed invoice dictionaries. Each dictionary may
            optionally contain a "source_file" key (e.g. the uploaded
            PDF's filename) used for traceability.
        output_path: Output .xlsx file path.
    """
    ...
    if position_rows:
        ...
        df_positions = pd.DataFrame(
            columns=[
                "source_file", "rechnungsnummer", "artikel", "menge",
