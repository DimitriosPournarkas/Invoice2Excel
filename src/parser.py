...
def _find_table_start(lines: list[str]) -> int | None:
    ...
    A header is recognized when a price-type column ("Preis"/"Gesamt"/
    "Betrag"/"Summe") appears together with at least one of: a
    description-type column ("Bezeichnung"/"Artikel"/...), a quantity-type
    column ("Menge"/"Anzahl"/"Stück"), or a period column ("Zeitraum").
    Not every invoice provider spells out all of these, so any single one
    of them next to the price column is treated as sufficient (e.g. some
    usage-based tables only have "Anzahl" and "Betrag", with no explicit
    description column header at all).
    """
...
def _parse_line_item(line: str) -> dict | None:
    ...
    """
    # Some invoices include a billing period somewhere in the row (e.g.
    # "01.05.26-31.05.26Paketpreis ..." or "Paketpreis 01.05.26-31.05.26
    # ..."), often glued directly to neighboring text due to PDF text
    # extraction. Detect and remove that period upfront, regardless of
    # where it sits in the line - otherwise the individual date numbers
    # (e.g. "31.05") can be misread as a monetary amount by the amount
    # matching below.
    ...
    if period_match:
        ...
        # Replace with a single space (instead of just deleting) so that
        # text on either side of the removed period doesn't get glued
        # together.
